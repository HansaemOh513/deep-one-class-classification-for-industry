...
class loader:
    def __init__(self, master=None, pattern_attention=None, premodeling=None):
        self.master = master
        self.pattern_attention = pattern_attention
        self.premodeling = premodeling
    ...
    def data_loader(self, path, resize, stop=False):
        '''
    ...
    def MS_loader(self, MS_list, resize = [False, 0, 0]): # Resize option, width, height
        '''
        인풋으로 MS_list를 받음. 예시 :
        ...
        def data_shape():
            for Mtype, step in MS_list:
                ...
                Mtype_data = self.data_loader(load_path, resize, stop=True)
                break
